...
rocon <- function(data, diagCol="black", roccol="red", isDecreasing=T, myTitle="ROC Curve") {
    ...
    p <- ggplot(data = rocdata, aes(x = FPR, y = TPR, color=method)) + 
      geom_line(size=.5) + 
      geom_line(data = diag, aes(x = x,y = y), color ="black") + 
      theme_bw() + 
      labs(x = "False Positive Rate",y = "True Positive Rate", title = myTitle) + 
      scale_color_discrete(labels=legLabs) + theme(legend.title=element_blank())

rocdata <- rocdata[1:2];
    return(list(rocdata, p, myAuc));
...
roconMult <- function(data, diagCol="black", colors=NULL, isDecreasing=T, myTitle="ROC Curve") {
    ...
    p <- ggplot(data = dfAll, aes(x = FPR, y = TPR, color=method)) +
      geom_line(size=.5) + 
      geom_line(data = diag, aes(x = x,y = y), color = "black") +
      theme_bw() + 
      labs(x = "False Positive Rate",y = "True Positive Rate", title = myTitle) + 
...
# another variation for paper
roconMult2 <- function(data, diagCol="black", colors=NULL, isDecreasing=T, myTitle="ROC Curve") {
