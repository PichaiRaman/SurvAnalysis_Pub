...
rocon <- function(data, diagCol="black", roccol="red", isDecreasing=T, myTitle="ROC Curve")
{
    ...
    p <- ggplot(data = rocdata, aes(x = FPR, y = TPR, color=method)) +geom_line(size=.5) + geom_line(data = diag, aes(x = x,y = y), color ="black")+theme_bw()+ labs(x = "False Positive Rate",y = "True Positive Rate", title = myTitle)+scale_color_discrete(labels=legLabs)+ theme(legend.title=element_blank())
